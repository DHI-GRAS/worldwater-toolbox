...
def _indices(path):
    ...
    # Calcualte indices of occurance, max and min extent
    ...
    water_occurrence.astype(np.float32).to_dataset(name = 'water_occurrence').rio.to_raster(path + '/index_water_occurrence.tif')
    ...
    classification.astype(np.float32).to_dataset(name = 'classification').rio.to_raster(path + '/index_classification.tif')

    return water_occurrence, min_extent, max_extent, classification
                                         

def _toolbox(connection, month_start, month_end, start, end, geometry, region, threshold, cloud_cover, rgb_processing): 
    ...
    ----------
    connection : STRING
        Connection oto manage and persist settings when interacting with the OpenEO API.
    ...
    geometry : Geojson 
        The path to the aoi file (geojson).

    region : STRING
        Choose an ecoregion from the options.

    threshold : INT
        Custom threshold use for the water probability layer 
    ...
    # Apply hillshade udf  
    process = openeo.UDF.from_file("world_water_toolbox/world_water_toolbox/udf.py", runtime = "Python")
    ...
    # Apply terrain correction for backscatter values
    ...
    my_job = output.create_job(title = 'water_'+ str(month_start.strftime("%Y_%m")) + '_' + str(month_end.strftime("%Y_%m")), out_format = "GTiff", job_options = {"node_caching":True},**{'filename_prefix': 'water_' + str(month_start.strftime("%Y_%m")) + '_' + str(month_end.strftime("%Y_%m"))})
    ...
    if rgb_processing:
         ...
         my_job = s2_cube_median.create_job(title = 'median_' + str(month_start.strftime("%Y_%m")) + '_' + str(month_end.strftime("%Y_%m")), out_format = "GTiff",job_options= {"node_caching":True},**{'filename_prefix': 'median_' + str(month_start.strftime("%Y_%m")) + '_' + str(month_end.strftime("%Y_%m"))})
         results = my_job.start_and_wait().get_results()
         results.download_files(output_folder)
...
def main(start, end, region, geometry, rgb_processing, cloud_cover, threshold):

    # Connect to openEO backends
    connection = openeo.connect("openeo-dev.vito.be").authenticate_oidc()

    # Check if the aoi exists 
    if os.path.isfile(geometry):
        print('Processing')

    else:
         print('The AOI file does not exit, please insert the correct geojson file')
         exit()
    ...
    # Iterrate toolbox for each month
    for month_start in months_list:
        month_end = month_start + relativedelta(months = 1) 
        output_folder = _toolbox(connection, month_start, month_end, start, end, geometry, region, threshold, cloud_cover, rgb_processing)


    # Calculate Calculate water frequency (0-1), minimum water extent, maximum water extent and water classification
    _indices(output_folder)

    print('Successfully finished! The output files are located:', output_folder)
